class OrdenCompra < ApplicationRecord
  ...
  accepts_nested_attributes_for :registro_materials, :reject_if => lambda {|a| a[:nombre_item].blank? && a[:sku].blank? && a[:cantidad].blank? && a[:precio].blank?}, :allow_destroy => true
end
